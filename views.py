    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            ActivityLog.objects.create(
                user=user, action="Account created", details="User registered with email: " + user.email)
...
class CustomLoginView(AuthLoginView, SafeMixin):
    ...
    def form_valid(self, form):
        ...
        ActivityLog.objects.create(
            user=user, action="User logged in", details="Login successful")
    ...
    def form_invalid(self, form):
        ...
        ActivityLog.objects.create(
            action="Failed login attempt", details=str(form.errors))
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if not TaskCompletion.objects.filter(user=request.user, task=task).exists():
                ActivityLog.objects.create(
                    user=request.user, action="Task completed", details=f"Task ID: {task_id}")
            ...
            ActivityLog.objects.create(
                user=request.user, action="Task completion failed", details=f"Task ID: {task_id} does not exist")
            return JsonResponse({'status': 'error', 'message': 'Task does not exist'}, status=404)
        except Exception as e:
            logger.error(f"Error completing task: {str(e)}")
            ActivityLog.objects.create(
                user=request.user, action="Task completion error", details=str(e))
...
class RegistrationSuccessView(TemplateView):
    ...
    def get(self, request):
        messages.success(
            request, "Your account was activated successfully. Welcome to the community sis.")
...
class BlogDetailView(DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if comment_form.is_valid():
            if request.user.is_premium_user() or request.user.is_highest_user():
                ...
                return HttpResponseForbidden("You must upgrade to comment on blogs.")
...
class ModeratorListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if user.is_staff:
            ...
            # Return threads where user is a moderator
            return Thread.objects.filter(moderators=user)
        else:
...
class ForumCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        if self.request.user.is_premium_user() or self.request.user.is_highest_user():
            form.instance.author = self.request.user
            return super().form_valid(form)
        else:
            return HttpResponseForbidden("You must upgrade to post in the forum.")
...
class CommentCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        if self.request.user.is_premium_user() or self.request.user.is_highest_user():
            ...
            return HttpResponseForbidden("You must upgrade to comment.")
...
class HabitCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        if self.request.user.is_free_user() and Habit.objects.filter(user=self.request.user).count() >= 3:
            return HttpResponseForbidden("Free users can only create up to 3 habits. Please upgrade your subscription.")
...
class IncrementHabitCounter(View):
    def post(self, request, pk):
        ...
        habit.increment_count()
        insight = habit.get_insights() if request.user.is_premium_user(
        ) or request.user.is_highest_user() else None
...
class ToDoCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if self.request.user.is_free_user() and ToDo.objects.filter(user=self.request.user).count() >= 5:
            return HttpResponseForbidden("Free users can only create up to 5 ToDos. Please upgrade your subscription.")
        form.instance.user = self.request.user
        return super().form_valid(form)
...
class JournalEntryCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, "Journal entry created successfully.")
        return super().form_valid(form)
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            overdue_todos = todos.filter(
                due_date__lt=now().date(), completed=False)
            ...
            context.update({
                ...
                'most_common_actions': ActivityLog.most_common_actions(user),
                'recent_activity': ActivityLog.recent_activity(user),
            })
        except Exception as e:
...
class QuestionCreateView(LoginRequiredMixin, CreateView):
    ...
    def dispatch(self, *args, **kwargs):
        if not self.request.user.is_premium_user() and not self.request.user.is_highest_user():
            return HttpResponseForbidden("You must be a premium or highest-tier subscriber to ask a question.")
