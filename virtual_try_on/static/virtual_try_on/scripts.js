document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateAvatarClothing(category, src) {
        ...
        // Move the old item back to the feature-options
        const oldItem = document.querySelector(`.draggable[data-src="${oldSrc}"]`);
        if (oldItem) {
            const parent = document.querySelector(`.options-category[data-category="${category}"] .options`);
            parent.appendChild(oldItem);
        }
    }

    window.resetAvatarClothing = function () {
        document.getElementById('avatar-top').src = '/static/virtual_try_on/garmets/tops/1.png';
        document.getElementById('avatar-skirt').src = '/static/virtual_try_on/garmets/skirts/1.png';
        document.getElementById('avatar-dress').src = '/static/virtual_try_on/garmets/dresses/1.png';
    ...
    window.saveAvatarClothing = function () {
        ...
        const skirt = document.getElementById('avatar-skirt').src;
        const dress = document.getElementById('avatar-dress').src;
        const shoes = document.getElementById('avatar-shoes').src;

        saveClothing('top', top);
        saveClothing('skirt', skirt);
        saveClothing('dress', dress);
