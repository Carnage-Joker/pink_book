...
class FileChange(_GitElement):
    ...
    def dump(self, file_):
        ...
        if self.type == b'M' and isinstance(self.blob_id, int):
            ...
            raise SystemExit(_("Unhandled filechange type: %s") %
...
class Tag(_GitElementWithId):
    ...
    def dump(self, file_):
        ...
        if (write_marks and self.id):
            file_.write(b'mark :%d\n' % self.id)
        markfmt = b'from :%d\n' if isinstance(
            self.from_ref, int) else b'from %s\n'
        file_.write(markfmt % self.from_ref)
        if self.tagger_name:
            file_.write(b'tagger %s <%s> ' %
            ...
            if suffix not in '1234567890':
                ...
                if suffix not in mult:
                    raise SystemExit(_("Error: could not parse --strip-blobs-bigger-than"
                                       " argument %s")
                                     % args.max_blob_size)
                args.max_blob_size = int(
                    args.max_blob_size[0:-1]) * mult[suffix]
            else:
        ...
        with open(filename, 'br') as f:
            for line in f:
                ...
                if line.startswith(b'regex:'):
                    ...
                    if repl:
                        raise SystemExit(
                            _("Error: In %s, 'glob:' and '==>' are incompatible (renaming globs makes no sense)" % decode(filename)))
                else:
                    if line.startswith(b'literal:'):
                        match = line[8:]
                    else:
...
class RepoAnalyze(object):
    ...
    def gather_data(args):
        ...
        dtp = subproc.Popen(cmd, shell=True, bufsize=-
        ...
        if dtp.wait():
            raise SystemExit(
                _("Error: rev-list|diff-tree pipeline failed; see above."))  # pragma: no cover
    ...
    def write_report(reportdir, stats):
        ...
        with open(os.path.join(reportdir, b"blob-shas-and-paths.txt"), 'bw') as f:
            f.write(("=== %s ===\n" % _(
                "Files by sha and associated pathnames in reverse size")).encode())
            f.write(
                _("Format: sha, unpacked size, packed size, filename(s) object stored as\n").encode())
            for sha, size in sorted(stats['packed_size'].items(),
                ...
                if len(names_with_sha) == 1:
                    ...
                    names_with_sha = b'[' + \
                        b', '.join(sorted(names_with_sha)) + b']'
...
class RepoFilter(object):
    ...
    def _run_sanity_checks(self):
        ...
        self._config_settings = GitUtils.get_config_settings(
            target_working_dir)
    ...
    def _prunable(self, commit, new_1st_parent, had_file_changes, orig_parents):
        ...
        for change in commit.file_changes:
            ...
            parent = new_1st_parent or commit.parents[0]
        ...
        def newname(path_changes, pathname, use_base_name, filtering_is_inclusive):
            ...
            for (mod_type, match_type, path_exp) in path_changes:
                if mod_type == 'filter' and not wanted:
                    ...
                    assert match_type in ('match', 'regex')
    ...
    def _tweak_commit(self, commit, aux_info):
        ...
        if not commit.dumped:
            if not self._prunable(commit, new_1st_parent,
                ...
                if self._args.state_branch:
                    alias = Alias(commit.old_id or commit.id,
                                  rewrite_to or deleted_hash)
    ...
    def _load_marks_file(self, marks_basename):
        ...
        if contents:
            biggest_id = max(int(x.split()[0][1:])
    ...
    def _save_marks_files(self):
        ...
        subproc.call(['git', '-C', working_dir,
                     'update-ref', full_branch, commit])
    ...
    def _setup_input(self, use_done_feature):
        if self._args.stdin:
            ...
            if self._args.dry_run or self._args.debug:
                ...
                if self._args.debug:
                    tmp = [decode(x) if isinstance(x, bytes)
                           else x for x in fep_cmd]
    ...
    def _migrate_origin_to_heads(self):
        ...
        if p.wait():
            raise SystemExit(
                _("git update-ref failed; see above"))  # pragma: no cover
    ...
    def _ref_update(self, target_working_dir):
        ...
        actual_renames = {k: v for k,
                          v in self._commit_renames.items() if k != v}
    ...
    def _record_metadata(self, metadata_dir, orig_refs):
        ...
        with open(os.path.join(metadata_dir, b'already_ran'), 'bw') as f:
            f.write(
                _("This file exists to allow you to filter again without --force.\n").encode())
    ...
    def run(self):
        ...
        if self._input:
            ...
            if not self._args.stdin and self._fep.wait():
                raise SystemExit(
                    _("Error: fast-export failed; see above."))  # pragma: no cover
            self._input.close()
        ...
        if not self._args.dry_run and self._fip.wait():
            raise SystemExit(
                _("Error: fast-import failed; see above."))  # pragma: no cover
